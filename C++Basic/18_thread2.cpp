// 스레드의 특성
// 1. pthread_create
//   : 스레드를 생성하고, 스레드가 만들어져서 시작할 함수를
//     지정한다.
// 2. 스레드는 자신만의 스텍을 가지고 있다.
// 3. 모든 스레드는 같은 프로세스 내에서 동작하기 때문에
//    데이터를 쉽게 공유할 수 있다.
#include <iostream>
#include <pthread.h>
using namespace std;
...
void *thread_routine(void *arg){
    ...
    for(int i=0; i<1000000; i++){
        ...
        //  : 오직 한개의 스레드만 접근 가능하도록 해야 한다.
        //      - 상호 배제(MUTual EXclustion)
        //          : Mutex
        // 원인
        // : 두개 이상의 스레드가 sum이라는 메모리에 연산을
        //   동시에 수행할 경우, 레이스 컨디션이 발생한다.
...
*/
// 4. 프로세스가 처음 만들어졌을 때, main 함수를 실행하는 흐름을 '메인 스레드'라고
//    부른다.
