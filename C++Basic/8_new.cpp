...
void foo(){
    ...
    // > foo함수가 처음 실행될 때 생성되고, 프로그램이 종료할 때까지 유지된다.
    //   : 함수가 상태를 가져야할 때 사용한다.
}
...
// 동적 메모리 할당
// - 힘(자유 영역)에 생성되는 변수이다.
//   > 개발자가 원하는 시점에 생성할 수 있고, 개발자가 원하는 시점에 파과할 수 있다.
//   1) 메모리 누수의 위험성이 있다.
//   2) 성능 오버헤드가 있다.
...
//    new는 전달 받은 타입의 주소로 반환하기 때문에 별도의 캐스팅이 필요하지 않는다.
// 4. malloc은 free로 해지하지만, new는 delete를 통해 해지해야 한다.
